...
import { uid, esc, saveFile, dateLabel, errorMessage } from '../utils.js';
  ...
  const render = () => {
    ...
    if (tab === 'api') {
      ...
      content.querySelector('[data-action="export-archive"]').onclick = run(async () => {
        ...
        await saveFile(
          blob,
          `paper-bridge-${new Date().toISOString().slice(0, 10)}.${password ? 'pbak' : 'zip'}`,
        );
        toast('备份已生成');
      });
      ...
      content.innerHTML = `<h3>阅读在设备间延续</h3><p class="muted">使用支持浏览器 CORS 与 ETag 的 WebDAV 服务。PDF、批注、译文和对话会合并同步；密钥留在本机。</p><form id="cloud-form" class="settings-form"><label class="field"><span>WebDAV 服务地址</span><input name="url" type="url" value="${esc(c.url)}" placeholder="https://dav.example.com" required></label><div class="field-pair"><label class="field"><span>用户名</span><input name="username" value="${esc(c.username)}" autocomplete="username"></label><label class="field"><span>密码 / 应用密码</span><input name="password" type="password" value="${esc(c.password)}" autocomplete="new-password"></label></div><label class="field"><span>同步目录</span><input name="path" value="${esc(c.path)}"></label><label class="toggle-row"><span>自动同步<small>每 30 分钟在应用打开期间同步</small></span><input name="enabled" type="checkbox" ${c.enabled ? 'checked' : ''}><span class="switch"></span></label><p class="note" id="sync-status">${c.lastSyncAt ? `上次同步：${dateLabel(c.lastSyncAt)}` : '尚未同步'}</p><div class="form-actions">${button('test-cloud', 'refresh-cw', '测试连接')}${button('save-cloud', 'check', '保存')}${button('sync-cloud', 'cloud', '立即同步', 'primary')}</div></form>`;
