...
适用版本：0.1.1。入口为 `index.html` → `src/js/main.js`。这是静态前端工程，**没有 `main.py`，也没有 Python 运行时或后端函数**；与原需求中 main.py 对应的应用协调职责由 `App` 类承担。
...
| annotations | id, documentId, page, type, color, rects/points/x/y/text, selectedText?, deleted, recovered? | 对特定 PDF 的归一化坐标；下划线/高亮/新批注保存选区 rects；局部移除裁剪 rects，清空使用 tombstone，支持同步与撤销 |
...
- `captureSelection()`：裁切真实 Range 与每个文字 span 的交集，转换归一化坐标；只更新操作按钮状态并请求翻译，不自动加标记。
...
- `registerSelectionAction(type,create)`：注册创建回调；返回额外字段，或返回 `null` 取消。当前注册 underline/highlight/note。未来同类工具调用注册方法并添加工具入口即可复用状态、删除和执行逻辑。
...
| setTool / colorPicker | 选区动作分发或持续工具模式、自绘颜色浮层 |
