...
- `App` 挂载时初始化一次；`renderTabs` 跳过相同内容的重绘，避免原页码自动保存回调重建卡片、中断正在进行的滚动；卡片内容变化时保留位置并通知当前文件。激活文件改变时仅滚动卡片视口以展示它，新文件位于尾部，溢出时自动靠右完整显示。鼠标移动超过 5px 才捕获指针，拖动后的点击被拦截，普通切换/关闭照旧；触摸与触控板使用原生滚动，键盘支持方向键和 Home/End，减少动态效果时按钮取消平滑动画。
- `openSettings` 添加专用 `.settings-modal`，原通用弹窗不变；仅 `#settings-content` 滚动，桌面保留左右两列，移动端为固定五项导航加内容两行。导航切页重置内容滚动位置，保存与字段语义保持原实现。
- 样式共用设计 token，桌面按钮和弹窗高度在 desktop.css，手机触控按钮及顶部五项导航在 mobile.css。新增 Lucide 0.468.0 原版 `triangle.svg`，CSS 旋转复用两个方向；下载脚本及许可证来源同步，没有新增 npm 依赖。
- 新增 `tests/e2e/layout-overflow.spec.js`，覆盖多文件打开/关闭、拖拽防误触、左右及键盘滚动、新卡片定位、宽窄切换、真实触摸滑动、长设置页导航可视与切页。通过 `PAPER_BRIDGE_SMOKE_ENGINE=firefox/webkit` 可切换内核；使用合成 PDF 和隔离资料，关闭标签仍保留文档。
- 验证：三内核各六项布局专项全部通过；Chromium 同时以协议真实触摸事件验证手机横向滑动。Windows WebKit 的临时匿名上下文不能写入 Blob，沿用项目既有临时持久资料方式后六项全部通过，未修改存储实现。语法、98 项单元、19 项原文件管理/编辑/设置自动保存回归、构建及生产 PDF 回归通过；dist 仍为 305 文件、约 89.4 MiB，版本、依赖、离线模型加载及 SW 策略不变。
- 完整端到端首轮 131 项通过、3 项可选联网/大模型条件未启用而跳过、4 项失败。其中首次 PDF 加载超过旧 5 秒断言，临时配置延长等待后原用例通过；两项缩放宽度检查在工具栏重绘间隙取得 null，临时副本对同一宽度断言等待重绘后桌面/手机均通过，原功能与原测试文件未修改。剩余有道说明测试期望“网页版不支持”，但任务开始前 HEAD 已是“机翻模型配置”；通过 `git show HEAD` 确认原有文案/断言不一致，保留现有文案并记录，未宣称完整套件全绿。
...
- 按最新要求将浮动按钮最大字号由 144 降到 48 pt，工具栏滑块和启动读取范围保持 6–48。既有大字号对象不被后台改写，主动调整时遵守新范围；对象修改仍使用原保存及撤销流程。
- 原 PdfNavigation.onMode 回调同步 viewer.setSearchHighlightsVisible，只有 search 模式显示搜索层；关闭或切换导航页签时隐藏，查询和结果缓存保留。新渲染页面继承当前可见性，关闭后滚动/缩放不会重新露出旧搜索高亮；用户自己添加的标记不受影响。
- 新增桌面/手机回归覆盖两种编辑模式、字号最大值、独立保存、新建/重启、菜单数值及即时滑块同步；查找覆盖页签切换、关闭/重开和隐藏期间缩放。版本保持 0.3.4，无数据库/存档结构或 UI 布局变动。

验证通过：82 项单元测试、10 项对象字号/查找/原编辑定点浏览器回归、JavaScript 语法检查、静态构建与生产子路径检查。桌面/手机均验证 48 pt 不再增加、15 pt 文本框与 48 pt 批注独立保存、重载后沿用和打开菜单时即时同步。首轮关闭测试错误使用只负责打开查找的工具按钮，改用原侧栏关闭按钮后显隐/缩放/恢复测试通过；业务关闭方式未改动。

## v0.3.4 同版本修订 · 橡皮擦连续移动命中 · 2026-09-24

- 根因是 bindInk 的 eraser 分支只在 pointerdown 查找一个对象，pointermove 仅处理笔迹/形状创建。修复限定在橡皮擦手势与擦除命中函数：按下即捕获活动指针，移动及松手末段持续检测，可从空白处开始并在一次按住期间擦除多个对象。
- shapes.hitEraserSweep 对事件间线段做几何命中，避免快速移动跨过细线；保留既有矩形/圆形内部可命中的规则及等效 18 CSS px 容差，仅允许 pen/shape。未按下、其他指针、取消/丢失捕获，以及切换工具/文档/缩放后的旧手势不再擦除。
- 用手势 seen 与 viewer.pendingEraseIds 去重，擦除通过原 writeAnnotations 队列事务保存，成功后更新显示与每对象撤销记录；失败不隐藏对象，撤销等待正在提交的擦除。原笔迹创建、文本输入、对象拖动、UI 和版本号不变。
- 新增几何与桌面/触控回归，覆盖空白起步、稀疏事件跨越、多对象、重复扫过、悬停、非活动指针、取消/失去捕获、保护其他标注、逐对象撤销/重做与刷新持久化。同步四份文档，保留已有未提交的 v0.3.4 修改。

本轮验证通过：JavaScript 语法检查、82 项单元测试、5 项橡皮擦/原编辑定点浏览器回归、静态构建及生产子路径检查。测试使用临时合成 PDF；没有修改版本、数据库或存档格式。

## v0.3.4 · 无后缀 PDF 外链与独立阅读字号 · 2026-09-24

范围仅为外链 URL 校验、比例控件宽度及原文/划词译文/全文译文三块的阅读字号。版本及包锁/界面同步 0.3.4，数据库、存档格式及运行依赖不变；复用既有下载图标 a-arrow-down/up。

- `parsePdfLink` 删除 URL 后缀及 query/hash 限制，仍只允许无嵌入账号密码的 HTTP(S) 地址；URL 路径末段生成安全文件名，保留固定 .pdf 后缀和可选重命名。`OpenPdfMenu.openLink` 继续 fetch→Blob/File→importFiles：不调用本地下载或上传选择器，PDF.js 验证真实内容后才入库；重定向、取消、原始 MD5、重复确认、根目录入库均沿用原流程。
- 用户给出的 arXiv 示例从真实本地浏览器直接 fetch 返回 200、application/pdf、1429059 字节和 %PDF-1.5 文件头；只在内存读取，无本地另存。自动回归用可控响应及本地 HTTP 302 服务验证无后缀、查询参数、重定向、application/octet-stream 和 HTML 拒绝，不以 Content-Type 或文件名冒充有效 PDF。
- `readingFontSizes` 保存 source/selection/full 三个百分比，默认 100，70–180 限制、每次 10；normalize 兼容旧/异常配置，adjustReadingFontSize 通过已有设置队列在提交时合并，避免快速点击和并发配置覆盖。随 settings/app 自然进入备份/同步，恢复遵循原 restoreSettings 开关。
- 新增 `ui/reading-fonts.js`，统一按钮、委托点击、CSS 比例和边界 disabled；Assistant 在切页、词典/流式/历史重绘后应用最新尺寸，settings-changed 仅修改阅读样式不重发翻译。reading-output 范围内缩放 Markdown/词典层级，绝对内联字号乘同一变量，相对 em/% 与 KaTeX 继续继承；不改正文内容、聊天或导出排版。
- 比例选择器使用固定 rem 宽度、移动端单独尺寸，展开菜单独立稳定宽度；保留七项选项及非标准数值显示。新增按钮沿用原复制/朗读的图标风格，窄屏按钮加大触控高度，全文结果动作栏必要时换行。

验证完成：语法检查、79 项单元、107 项浏览器用例（全量 105 项通过；两项旧用例补齐配置保存/刷新恢复的异步等待后定点通过）、构建、Chromium 和 WebKit 18.4 生产子路径检查。新增六项浏览器用例覆盖桌面/手机字号独立、边界禁用、切换论文/刷新/历史/流式持续、Markdown/内联字号、菜单宽度、真实 302 跳转及 HTML 拒绝；字号设置还验证无密钥存档和显式恢复设置。真实 arXiv 示例通过 APP 窗口导入为 2503.13443.pdf、16 页、根目录，下载事件/文件选择器计数均为零。旧 WebKit 18.4 实测 20px 内联文字调为 22px，三个比例选项控件宽度均为 88.1875px；桌面/手机截图已检查。最终 dist 共 283 文件、约 16.5 MiB，测试使用临时浏览器资料，原 5173 服务保留。
